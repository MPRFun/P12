...
if (ClickShake == false)
	...
	tooltip7 := A_ScreenHeight/3+(tooltipSpacing*10)
...
if (ErrorLevel == 0)
	...
	if (xxx < ModernMaxLeft)
		...
		if (ErrorLevel == 0)
			{
			xx := xx+Offset
			Difference := (xxx-xx)*StableScaling
			if (Difference >= 0)
				{
				if (Difference < 10)
					Difference := 10
					...
					Difference := Offset
				Difference := Difference*ResolutionScaling
				tooltip, >, %xx%, %FishTrackHeight%, 19
				send {lbutton down}
				sleep %Difference%
				send {lbutton up}
				Difference := Difference/1.3333
				sleep %Difference%
				GoingRight := true
				if (GoingLeft == true)
					{
				...
				{
				Difference := (xx-xxx)*StableScaling
				if (Difference < 10)
					Difference := 10
				else if (Difference > Offset)
					Difference := Offset
			...
			if (ErrorLevel == 0)
				...
				if (difference < 0)
					...
					send {lbutton up}
					Difference := Offset*ResolutionScaling*UnstableScaling
					sleep %Difference%
					Difference := Difference/1.3333
					send {lbutton down}
					sleep %Difference%
